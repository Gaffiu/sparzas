...
export default function MobileTabBar() {
  ...
  const tabs = [
    ...
    { path: '/subscriptions', icon: '📺', label: 'Inscrições' },
    { path: '/upload', icon: '＋', label: 'Publicar', requireAuth: true },
    { path: user ? `/channel/${user.id}` : '/login', icon: '👤', label: 'Canal' },
  ];
  ...
  return (
    <nav style={{
      position: 'fixed', bottom: 0, left: 0, width: '100%', height: 72,
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            style={{
              background: 'none', border: 'none', color: isActive ? '#00e676' : '#aaa',
