...
import { IconHome, IconShorts, IconUpload, IconSubscriptions, IconLibrary } from './Icons';
...
export default function MobileTabBar() {
  ...
  const tabs = [
    ...
    { path: '/shorts', icon: IconShorts, label: 'Shorts' },
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button key={tab.path} onClick={() => handleTab(tab)} className={`tab-btn ${isActive ? 'active' : ''}`}>
            <IconComponent size={24} color={isActive ? '#00e676' : '#888'} />
            <span>{tab.label}</span>
          </button>
        );
      })}
